...
import { formatCurrency, formatMT, getFYMonths, getFYFromDate, calculateRateWithGst, calculateBasicRateFromInclusive, calculateInvoiceFinalAmount, calculateInvoiceItemsTotals, calculateNoteTaxBreakdown } from '@/lib/calculations'
...
export default function PurchaseReturnPage({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const itemGstPct = returnItems.length > 0 ? (typeof items.find(i => i.id === returnItems[0].itemId)?.gstRate === 'number' ? items.find(i => i.id === returnItems[0].itemId)!.gstRate! : gstPercentage) : gstPercentage

    const noteTaxBreakdown = calculateNoteTaxBreakdown({
      amount: calculatedTotalAmount,
      isTaxInclusive: true,
      gstRate: itemGstPct,
      partyStateCode,
    ...
    const debitNoteRecord: SupplierDebitNote = {
      id: debitNoteId,
      noteNo: finalReturnNo,
      supplierId: selectedSupplierId,
      date: returnDate,
      amount: calculatedTotalAmount,
      invoiceRef: finalReturnNo,
      originalInvoiceNo: finalReturnNo,
      originalInvoiceDate: returnDate,
      reason: '01 - Purchase Return',
      ...
      cgstRate: noteTaxBreakdown.cgstRate,
      cgstAmount: noteTaxBreakdown.cgstAmount,
      sgstRate: noteTaxBreakdown.sgstRate,
      sgstAmount: noteTaxBreakdown.sgstAmount,
      igstRate: noteTaxBreakdown.igstRate,
      igstAmount: noteTaxBreakdown.igstAmount,
      roundOff: noteTaxBreakdown.roundOff,
      totalAmount: noteTaxBreakdown.totalAmount,
      isInterState: noteTaxBreakdown.isInterState,
      isTaxInclusive: true,
    ...
    if (activeCompanyId) {
      void saveEntityRemote(activeCompanyId, 'purchaseReturns', purchaseReturnRecord)
      void saveEntityRemote(activeCompanyId, 'debitNotes', debitNoteRecord)
    }
  ...
  const handleDelete = () => {
    ...
    if (activeCompanyId) {
      void deleteEntityRemote(activeCompanyId, 'purchaseReturns', returnId)
      void deleteEntityRemote(activeCompanyId, 'debitNotes', debitNoteId)
    }
