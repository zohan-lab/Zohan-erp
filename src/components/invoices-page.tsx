...
import { formatCurrency, roundCurrency, formatMT, getFYMonths, getFYFromDate, calculatePaymentAllocations, calculateRateWithGst, calculateBasicRateFromInclusive, calculateRoundOffAdjustment, calculateInvoiceFinalAmount, calculateCostBreakdownDetails, calculateInvoiceItemsTotals, calculateAdditionalChargesTotals, calculateInvoiceTaxBreakdown, isInterStateTransaction, calculateInvoiceTotals } from '@/lib/calculations'
...
export default function InvoicesPage({
  ...
  const currentSupplier = selectedInvoiceSupplier
  const taxSummary = useMemo(() => {
    ...
    return calculateInvoiceTaxBreakdown({
      ...
      additionalCostFinal,
      partyState: currentSupplier?.stateCode || currentSupplier?.stateName || currentSupplier?.state,
      customRoundOff: roundOffAdjustment,
      defaultGstRate: gstPercentage
    })
  }, [invoiceItems, items, additionalCharges, additionalCostFinal, currentSupplier?.stateCode, currentSupplier?.stateName, currentSupplier?.state, roundOffAdjustment, gstPercentage])
                              ...
                              return (
                                <div key={charge.id} className="erp-charge-dashed-card space-y-2">
                                  ...
                                  <div className="erp-charge-row-inputs">
                                    ...
                                    <Select value={effectiveMode} onValueChange={(value) => handleUpdateCharge(charge.id, 'taxMode', value)}>
            ...
            <TableBody>
              {filteredInvoices.length === 0 ? (
                ...
                filteredInvoices
                  ...
                  .map((invoice) => {
                    const supplier = supplierMap.get(invoice.supplierId) || suppliers.find(s => s.id === invoice.supplierId || s.name.toLowerCase() === invoice.supplierId.toLowerCase())
                    ...
                    return (
                      <TableRow key={invoice.id} className="hover:bg-slate-50/80 border-b border-slate-100">
                        ...
                        <TableCell className="font-semibold text-slate-800 text-sm">{supplier?.name || invoice.supplierId || 'Unknown'}</TableCell>
