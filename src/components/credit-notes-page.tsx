...
import { CreditNote, Party, SalesInvoice, PurchaseInvoice } from '@/lib/types'
...
export const STATUTORY_NOTE_REASONS = [
  { code: '01', label: '01 - Sales / Goods Return' },
  { code: '02', label: '02 - Post Sale / Rate Discount' },
...
interface CreditNotesPageProps {
  creditNotes: any[]
  setCreditNotes: (updater: (prev: any[]) => any[]) => void
  parties: Party[]
...
export default function CreditNotesPage({
  ...
  const [reason, setReason] = useState('01 - Sales / Goods Return')
  ...
  }, [selectedPartyId, salesInvoices, invoices])
  ...
  const handleOpenDialog = (item?: any) => {
    if (item) {
      ...
      setSelectedPartyId(item.partyId || item.customerId || item.supplierId || '')
      setDate(item.date || new Date().toISOString().split('T')[0])
      setAmount((item.totalAmount ?? item.amount ?? 0).toString())
      setReason(item.reason || '01 - Sales / Goods Return')
      ...
      setNoteNo(`CN-${format(new Date(), 'yyyyMMdd')}-${String(nextNum).padStart(3, '0')}`)
      setSelectedPartyId('')
      setDate(new Date().toISOString().split('T')[0])
      setAmount('')
      setReason('01 - Sales / Goods Return')
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!noteNo.trim()) {
      toast.error('Please enter a Credit Note number')
      return
    }
    ...
    const notePayload: CreditNote = {
      id: editingItem ? editingItem.id : `cn-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      noteNo: noteNo.trim(),
    ...
    if (editingItem) {
      setCreditNotes(prev => prev.map(cn => cn.id === editingItem.id ? notePayload : cn))
      toast.success(`Credit Note "${notePayload.noteNo}" updated successfully`)
    } else {
      setCreditNotes(prev => [notePayload, ...prev])
      toast.success(`Credit Note "${notePayload.noteNo}" created successfully`)
    }

    if (activeCompanyId) {
      saveEntityRemote(activeCompanyId, 'creditNotes', notePayload).catch(err => {
        console.warn('Firebase dual-sync for Credit Note warning:', err)
      })
    }
  ...
  const handleDelete = async () => {
    ...
    toast.success(`Credit Note "${noteNum}" deleted`)

    if (activeCompanyId) {
      deleteEntityRemote(activeCompanyId, 'creditNotes', id).catch(err => {
        console.warn('Firebase delete Credit Note warning:', err)
      })
    }
  ...
  const filteredList = useMemo(() => {
    return creditNotes.filter(item => {
      ...
      const pid = item.partyId || item.customerId || item.supplierId
  ...
  }, [creditNotes, periodFilter, currentFY, selectedEntity, searchQuery, parties])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Create and track Credit Notes issued to or received from all counterparties.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border border-slate-200 bg-white shadow-xs">
          <CardContent className="p-4">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Credit Notes</div>
        ...
        <Card className="border border-slate-200 bg-white shadow-xs">
          <CardContent className="p-4">
            ...
            <div className="text-[11px] text-slate-400 mt-1">Total Credit Value</div>
          </CardContent>
        </Card>
      </div>
              ...
              {filteredList.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-12 text-slate-400 text-sm">
                    No Credit Notes found matching the selected filters.
                  </TableCell>
                </TableRow>
              ) : (
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold flex items-center gap-2">
              ...
              {editingItem ? 'Edit Credit Note' : 'New Credit Note'}
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSave} className="space-y-4 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold">Credit Note No *</Label>
                <Input
                  value={noteNo}
                  onChange={(e) => setNoteNo(e.target.value)}
                  placeholder="e.g. CN-20260401-001"
                  required
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <Button type="submit" disabled={isLocked} className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs">
                {editingItem ? 'Update Credit Note' : 'Create Credit Note'}
              </Button>
            </div>
          </form>
        </DialogContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete Credit Note <strong>{itemToDelete?.noteNo}</strong>? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
